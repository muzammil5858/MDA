...
@php
    ...
    $participantHtml = '<span class="name">الاٹیاں</span><span class="divider">/</span>'
    . '<span class="name">موہوب الیہم</span><span class="divider">/</span>'
    . '<span class="name">شرعی ورثاء</span><span class="divider">/</span>';

    $filtered = array_intersect_key($urduOrdinalsPlural, array_flip($indexesToGet));
    } else {
    $participantHtml = '<span class="name">الاٹی</span><span class="divider">/</span>'
    . '<span class="name">موہوب الیہ</span><span class="divider">/</span>'
    . '<span class="name">شرعی وارث</span><span class="divider">/</span>';
...
@php
  ...
  $text = '';
  $type = $urduOrdinals[$index-1] ?? ' الاٹی ';
...
تعدادی 
@if($property->kanal) {{ $property->kanal }} کنال @endif
@if($property->marla) {{ $property->marla }} مرلہ @endif
@if($property->sqft) {{ $property->sqft }} مربع فٹ @endif 

واقع سیکٹر {{ $property->sector }}, {{ $property->township->urdu_name }} 

تحت ضابطہ بنام {{ $property->owners->map(fn($p) => $p->name .' '. $p->father_name)->implode(' اور ') }}
{{$siffixs}} شدہ ہے۔ 

{{$type.' '}} {{ $request->participants->map(fn($p) => $p->owner->name. ' ' . $p->owner->father_name.' ')->implode(' اور ') }} 

کی جانب سے مختار عام مقرر ہے۔ مختار دہندہ زندہ حیات ہے اور مختارنامہ قابل عمل ہے۔
مظہر/مظہرہ/مظہران نے اب بذریعہ مختار عام یہ پلاٹ بطور {{$type12}} بالعوض 
<b>{{ number_format($data->amount) }} روپے</b> 
مُسمی {{ $request->dummyreceiver->map(fn($e) => $e->name .' ' . $e->father_name)->join('، ') }}
کو دے کر قبضہ حوالے <strong class="name" data-id="muzzamal"> {{$type1}}</strong> کر دیا ہے ۔ اس لئے اب <strong class="name" data-id="muzzamal"> {{$type}}</strong> 

             کو اس پلاٹ کی منتقلی بنام <strong class="name" data-id="muzzamal">{{$type1}}</strong> میں کوئی عذر اعتراض نہیں ہے اور اس پلاٹ کی نسبت کسی دیگر فرد/ادارہ کو مختار/اقرار نامہ
            معاہدہ بیع وغیرہ نہیں دے رکھا ہے اور نہ ہی اس پلاٹ کو رہن رکھ کر کسی بھی سرکاری وغیر سرکاری ادارہ سے قرض
            حاصل کر رکھا ہے۔ بیان ہذا سے قبل اگر اس پلاٹ کی نسبت کسی قسم کا ابہام ہوا تو مظہر/مظہرہ/مظہران ذمہ دار
            ہوگا/گی/ادارہ ذمہ دار نہ ہوگا۔ مظہر/مظہرہ/مظہران حلفاً بیان کرتا/ کرتی ہے کہ پلاٹ سے متعلق کاغذات/دستاویزات
            حوالہ
            <strong class="name" data-id="muzzamal">{{$type1}}</strong>
             کر دئیے ہیں۔استدعا ہیکہ پلاٹ بنام <strong class="name" data-id="muzzamal">{{$type1}}
                </strong>
            {{ $request->dummyreceiver->map(fn($e) => $e->name .' ' . $e->father_name)->join('، ')
            }} منتقل فرمایا جائے۔<br>مظہر/مظہرہ حلفاً بیان کرتا ہے کہ مظہر/مظہرہ کو معلوم و تسلیم ہے کہ پلاٹ کی منتقلی
            پر حکومت کی جانب سے ایڈوانس انکم ٹیکس،ایجوکیشن سیس،سٹیمپ ڈیوٹی وغیرہ کے نفاذ کے معاملہ پرپروفیسر(ریٹائرڈ)
            اذکار احمد وغیرہ بنام منگلا ڈیم ہاؤسنگ اتھارٹی میرپور وغیر ہ کے عنوان سے رٹ پٹیشن عدالت العالیہ میں زیرسماعت
            ہے اور معززعدالت نے حکم مصدرہ2022-10-19 کے تحت ایڈوانس انکم ٹیکس ، ایجوکیشن سیس، سٹیمپ ڈیوٹی کی وصولی معطل
            کی ہوئی ہے ۔ لہذا مظہر/مظہرہ پلاٹ زیر بحث کی نسبت جس قدر ٹیکسز درج بالا سابقہ و موجودہ واجب الادا ہوں گے
            معزز عدالت العالیہ کے احکامات کی روشنی میں ادا کرنے کا /کی پابند ہوگا/گی
        </p>
