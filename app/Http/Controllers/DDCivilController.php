...
class DDCivilController extends Controller
{
    public function pendingRequests()
    {
        ...
        $pendingApplications = Requests::with(['property', 'participants.owner', 'transfer'])
            ...
            ->where('ad_status', 1)
            ->where('dd_action', Null) // Assuming AD-Civil sees requests approved by HDM
            ->latest()
            ->paginate(15);

        return view('dd-civil.pendingRequests', compact('pendingApplications'));
    }
    ...
    public function approvedRequests()
    {
       $pendingApplications = Requests::with(['property', 'participants.owner', 'transfer'])
            ...
            ->latest()
            ->paginate(15);
        return view('dd-civil.acceptedRequest', compact('pendingApplications'));
    }
    ...
    public function viewPendingRequest($id)
    {
        // Logic to view details of a specific pending request for AD-Civil
         $smallRequest = SmallRequest::with(['property.owners', 'property.township'])->where('request_id', $id)->first();
                ...
                return view('dd-civil.actionPendingRequest', compact('smallRequest', 'request', 'property', 'mapApproval'));
    }
    public function submitPendingRequest(Request $request, $id)
    {
        ...
        return redirect()->route('dd-civil.pendingRequests')->with('success', 'Request has been approved and submitted successfully.');
    }
}
